...
- Limitation: Image embedding remains T18; T20's cooperative transaction-wide policy applies while Worker isolation remains T21; rendering remains T23. Layout, barcodes, Forms, tagged-document construction, SVG conversion, and redaction are not included.
...
- Limitation: Provider Operating Limits cover request bytes, result bytes, and elapsed subprocess time. T20 supplies the cooperative policy for the surrounding trusted in-process Document Workflow; complete subprocess containment and the Hardened Worker Profile remain T21.
...
- Limitation: T20 now layers finite-default transaction-wide resource accounting and cooperative checkpoints onto T03. Arbitrary callback/backend termination and hard hostile multi-tenant isolation remain T21.
...
- Limitation: T20 is only the trusted in-process profile. Hostile multi-tenant execution, process isolation and hard termination, process-tree containment, and network denial remain the T21 Hardened Worker Profile; runtime rendering remains T23.
...
- `authoritative-policy`: [`docs/hostile-input-policy.md`](../hostile-input-policy.md) — Defaults, accounting boundaries, local-limit composition, stable failures, cleanup, publication behavior, cooperative limits, and the T21 isolation boundary are documented.
...
- Limitation: Parsed COS inspection cannot recover duplicate raw dictionary keys, hexadecimal-versus-literal string delimiters, or other lexical details normalized or repaired by PDFBox. T20 bounds the parsed graph and supported decoding but cannot restore those raw distinctions; hard isolated parsing remains T21.
...
- Limitation: Public-key encryption, FIPS validation, signature creation or trust validation, and Hardened Worker isolation remain T37, T38+, and T21 scope. T20's cooperative transaction policy applies but is not hard isolation.
