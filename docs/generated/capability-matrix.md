...
- Limitation: T21 supplies no remote Worker, public backend SPI, renderer, crash-recovery identity, idempotent retry, lost-acknowledgement recovery, uncertain-publication lookup, large-value chunking, 5,000-page or 1-GiB certification, or behavior from downstream T22 and later tickets.
...
- `authoritative-contract`: [`docs/hardened-worker.md`](../hardened-worker.md) — Selection, callback placement, protocol, ordering, ownership, publication, credentials, Providers, exact classpath authority, supported Linux/JDK envelope, resource and isolation controls, cleanup, stable failures, operational limits, evidence boundary, and T22 exclusions are documented.
