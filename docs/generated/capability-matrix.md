...
- Limitation: Inspection limits are explicit per-query traversal and decoded-stream bounds and compose with T20's shared object, nesting, decompression, memory, time, storage, input, page, pixel, and concurrency policy.
- Limitation: T09 runs through the trusted in-process adapter by default; T21's opt-in Hardened Worker transports the same bounded PDF Value and Document Patch contract through closed version-1 codecs.
