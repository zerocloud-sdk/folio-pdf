...
final class MarkdownGenerator {
    ...
    private static String capabilityDocument(
        ...
        appendFoundationLink(text, model);
        ...
        for (InventoryModel.Capability capability : capabilities) {
            ...
            if (!capability.parentCapability.isEmpty()) {
                text.append("- Aggregate parent: ").append(code(capability.parentCapability)).append("\n");
            }
    ...
    private static void appendFoundationLink(StringBuilder text, InventoryModel model) {
        if (model.foundation != null) {
            text.append("Foundation 0.1.0 release obligations, exact required Ubuntu/JDK environments and current blockers: ")
                    .append("[Foundation readiness](foundation-readiness.md). ")
                    .append("Facade exclusions below record current missing coverage and cannot waive required Foundation mappings.\n\n");
        }
    }
    ...
    static String finishDocument(StringBuilder text) {
        int end = text.length();
