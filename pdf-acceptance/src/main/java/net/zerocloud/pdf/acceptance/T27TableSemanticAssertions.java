...
final class T27TableSemanticAssertions {
    private T27TableSemanticAssertions() { }
    ...
    static Observation inspect(WorkflowOutcome<Void> creation, Path artifact) {
        boolean receipt = creation.getPublicationReceipts().size() == 1
                ...
                && T27TableProducts.CAPABILITY.equals(creation.getCapabilityId());
        try {
            Observation observation = new DocumentWorkflow().execute(WorkflowRequest.open(artifact, SaveMode.REWRITE),
                    T27TableSemanticAssertions::observe).getResult();
    ...
    private static Observation observe(DocumentSession session) throws DocumentFailure {
        ...
        int[] offsets = new int[T27TableExpectations.PAGE_COUNT];
        if (geometry) {
            for (int p = 0; p < T27TableExpectations.PAGE_COUNT; p++) {
                geometry &= T27TableExpectations.TEXT[p].equals(pages.get(p).getText());
            }
            ...
            for (T27TableExpectations.Run run : T27TableExpectations.RUNS) {
                ...
                for (int i = 0; i < run.text.length(); i++) {
                    ...
                    geometry &= String.valueOf(cp).equals(item.getTextContribution())
                            ...
                            && near(value.getAdvanceX().doubleValue(),T27TableExpectations.advance(cp))
                    ...
                    x += T27TableExpectations.advance(cp);
                }
            }
            for (int p = 0; p < T27TableExpectations.PAGE_COUNT; p++) { geometry &= offsets[p] == pages.get(p).getTextItems().size(); }
        }
        List<FontResource> fonts = session.query(ExtractImagesAndResources.version1(
                ResourceExtractionLimits.builder().maximumPages(32).maximumPageTreeNodes(128)
        ...
        boolean borders = pages.size() == T27TableExpectations.PAGE_COUNT;
        if (borders) {
            for (int page = 1; page <= T27TableExpectations.PAGE_COUNT; page++) {
                ...
                for (double[] cell : T27TableExpectations.CELLS) {
                    if (cell[0] == page) {
                        for (double[] box : T27TableExpectations.borders(cell)) { expected.add(box); }
                    }
                }
    ...
    private static boolean near(double actual, double expected) {
        return Math.abs(actual - expected) <= T27TableExpectations.TOLERANCE;
    }
