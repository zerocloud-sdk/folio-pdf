 ...
 */
final class T31DataMatrixBitMatrixParser {

  private final BitMatrix mappingBitMatrix;
  private final BitMatrix readMappingMatrix;
  private final T31DataMatrixVersion version;

  /**
   * @param bitMatrix {@link BitMatrix} to parse
   * @throws FormatException if dimension is < 8 or > 144 or not 0 mod 2
   */
  T31DataMatrixBitMatrixParser(BitMatrix bitMatrix) throws FormatException {
  ...
  T31DataMatrixVersion getVersion() {
    return version;
  }
   ...
   * @return {@link T31DataMatrixVersion} encapsulating the Data Matrix Code's "version"
   * @throws FormatException if the dimensions of the mapping matrix are not valid
   * Data Matrix dimensions.
   */
  private static T31DataMatrixVersion readVersion(BitMatrix bitMatrix) throws FormatException {
    int numRows = bitMatrix.getHeight();
    int numColumns = bitMatrix.getWidth();
    return T31DataMatrixVersion.getVersionForDimensions(numRows, numColumns);
  }
  ...
  /** Exact fixed modules omitted from payload placement; original T31 assertion. */
  void requireUnusedModules() throws FormatException {
    int width = mappingBitMatrix.getWidth(), height = mappingBitMatrix.getHeight();
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (readMappingMatrix.get(x,y)) { continue; }
        if (x < width-2 || y < height-2 || mappingBitMatrix.get(x,y) != (width-x == height-y)) {
          throw FormatException.getFormatInstance();
        }
      }
    }
